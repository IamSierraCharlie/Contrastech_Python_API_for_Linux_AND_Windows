...
class CameraApi(object):
    ...
    def set_camera_resolution(self, cam_id, width: int, height: int, img_channels=3, centre_resolution=True):
        """
        @param self: A high level function to easily set your intended image resolution
        @param cam_pointer: this is the pointer of the camera you wish to target from the camera list
        ...
        """
        if self.camera_pointer_list[cam_id] is None:
            self.dprint('No camera - you must create a camera instance')
        else:
            self.img_width = self.check_width(cam_id, width)
            self.img_height = self.check_height(cam_id, height)
            self.img_channels = img_channels
            # you must sent the offsets back to zero here otherwise this will fail
            self.set_offset(cam_id, centre_resolution)
            self.property(cam_id, "Width", self.img_width)
            self.property(cam_id, "Height", self.img_height)

    def check_width(self, cam_id, width):
        max_width = self.property(cam_id, "SensorWidth")
    ...
    def check_height(self, cam_id, height):
        max_height = self.property(cam_id, "SensorHeight")
    ...
    def set_offset(self, cam_id, centre_resolution):
        max_width = self.property(cam_id, "SensorWidth")
        max_height = self.property(cam_id, "SensorHeight")
        ...
        self.property(cam_id, "OffsetX", 0)
        self.property(cam_id, "OffsetY", 0)
        ...
        self.property(cam_id, "OffsetX", offset_x)
        self.property(cam_id, "OffsetY", offset_y)
    ...
    def unsubscribe_camera_status(self, cam_pointer):
        ...
        event_subscribe_info.pCamera = pointer(cam_pointer)
    ...
    def subscribe_camera_status(self, cam_id):
        # Registration notification
        event_subscribe = pointer(GENICAM_EventSubscribe())
        event_subscribe_info = GENICAM_EventSubscribeInfo()
        event_subscribe_info.pCamera = pointer(self.camera_pointer_list[cam_id])
        n_ret = GENICAM_createEventSubscribe(byref(event_subscribe_info), byref(event_subscribe))
        if n_ret != 0:
    ...
    def open_camera(self, cam_id):
        n_ret = self.camera_pointer_list[cam_id].connect(self.camera_pointer_list[cam_id], c_int(GENICAM_ECameraAccessPermission.accessPermissionControl))
        if n_ret != 0:
            self.dprint("camera connect fail!")
            return -1
        else:
            self.dprint("camera connect success.")
        n_ret = self.subscribe_camera_status(cam_id)
        if n_ret != 0:
            self.dprint("subscribeCameraStatus fail!")
            return -1
        return 0

    # NOTE: if the cameras are slightly different, then the xml files need to be different too.
    # This is not handled here yet
    def get_xml(self, cam_id):
        xml = c_char_p()
        n_ret = self.camera_pointer_list[cam_id].downLoadGenICamXML(self.camera_pointer_list[cam_id], xml)
        self.dprint(n_ret)
        if n_ret != 0:
    ...
    def close_camera(self, cam_id):
        n_ret = self.unsubscribe_camera_status(self.camera_pointer_list[cam_id])
        if n_ret != 0:
            self.dprint("unsubscribeCameraStatus fail!")
            return -1
        n_ret = self.camera_pointer_list[cam_id].disConnect(byref(self.camera_pointer_list[cam_id]))
        if n_ret != 0:
    ...
    def connect_device_control(self, cam_id):
        device_control = pointer(GENICAM_DeviceControl())
        device_control_info = GENICAM_DeviceControlInfo()
        device_control_info.pCamera = pointer(self.camera_pointer_list[cam_id])
        ...
        string_parser_info.pCamera = pointer(self.camera_pointer_list[cam_id])
    ...
    def get_usb_info(self, cam_id):
        ...
        usb_camera_info.pCamera = pointer(self.camera_pointer_list[cam_id])
    ...
    def get_camera_list(self):
        ...
        if n_ret != 0:
            ...
            self.dprint("Only 1 camera was found, perhaps you should use the camsApi instead of multicamsApi")
            self.camera_count = camera_cnt.value
            self.camera_pointer_list = camera_list
            return camera_cnt.value, camera_list
        else:
            self.dprint("Multiple cameras found - here comes the list!")
            self.camera_count = camera_cnt.value
            self.camera_pointer_list = camera_list
            return camera_cnt.value, camera_list

    def create_camera_instance(self, cam_id=0):  # from the camera list
        ...
        # self.dprint("\nCamera Id = " + str(index))
        self.dprint("vendor name   = " + str(self.camera_pointer_list[cam_id].getVendorName(self.camera_pointer_list[cam_id])))
        self.dprint("Model  name   = " + str(self.camera_pointer_list[cam_id].getModelName(self.camera_pointer_list[cam_id])))
        self.dprint("Serial number = " + str(self.camera_pointer_list[cam_id].getSerialNumber(self.camera_pointer_list[cam_id])))
        self.dprint("Device Version = " + str(self.camera_pointer_list[cam_id].getDeviceVersion(self.camera_pointer_list[cam_id])))
        self.dprint("Interface name = " + str(self.camera_pointer_list[cam_id].getInterfaceName(self.camera_pointer_list[cam_id])))
        self.dprint("Interface type = " + str(self.camera_pointer_list[cam_id].getInterfaceType(self.camera_pointer_list[cam_id])))
        fname = f"{(self.camera_pointer_list[cam_id].getVendorName(self.camera_pointer_list[cam_id])).decode('utf-8')}_{(self.camera_pointer_list[cam_id].getModelName(self.camera_pointer_list[cam_id])).decode('utf-8')}_{cam_id}"
        zippedfilename = f"{fname}.zip"
        xmlfile = f"{fname}.xml"
        charzip = c_char_p(zippedfilename.encode())
        self.camera_pointer_list[cam_id].connect(self.camera_pointer_list[cam_id], c_int(GENICAM_ECameraAccessPermission.accessPermissionControl))
        # TODO: check to see if this is successful - if its not, then we cannot access the camera
        n_ret = self.camera_pointer_list[cam_id].downLoadGenICamXML(self.camera_pointer_list[cam_id], charzip)
        if n_ret != 0:
    ...
    def genicam_worker(self, cam_pointer, camera_parameter, parameter_value, camera_xml_file):
                        ...
                        for subtags in enumsubtags:
                            if subtags.text != "Invisible":
                                if camera_parameter == subtag.attrib["Name"]:
                                    ...
                                    node_pointer_info.pCamera = pointer(cam_pointer)
    ...
    def property(self, cam_id, camera_parameter, parameter_value=None):  # type will be grabbed from the xml
        ...
        result, node_pointer, node_pointer_info, p_value, node_type = self.genicam_worker(
            self.camera_pointer_list[cam_id], camera_parameter, parameter_value, self.xml_property_file)
    ...
    def set_roi(self, cam_id, offset_x, offset_y, n_width, n_height):  # another example from the Contrastech Demo
        # set the pointer
        ...
        width_max_node_info.pCamera = pointer(self.camera_pointer_list[cam_id])
        ...
        height_max_node_info.pCamera = pointer(self.camera_pointer_list[cam_id])
        ...
        width_node_info.pCamera = pointer(self.camera_pointer_list[cam_id])
        ...
        height_node_info.pCamera = pointer(self.camera_pointer_list[cam_id])
        ...
        offset_x_node_info.pCamera = pointer(self.camera_pointer_list[cam_id])
        ...
        offset_y_node_info.pCamera = pointer(self.camera_pointer_list[cam_id])
